...
import { Users, UserPlus, Mail, Phone, Briefcase, CheckCircle2, XCircle, Search } from 'lucide-react'
...
export default function SupervisorExpertsPage() {
  ...
  const fetchExperts = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/supervisor/experts', {
        credentials: 'include'
      })
  ...
  const toggleExpertStatus = async (expertId: string, currentStatus: boolean) => {
    if (!confirm(`هل تريد ${currentStatus ? 'تعطيل' : 'تفعيل'} هذا الخبير؟`)) return
    ...
    try {
      ...
      if (response.ok) {
        alert(`تم ${currentStatus ? 'تعطيل' : 'تفعيل'} الخبير بنجاح`)
        fetchExperts()
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#c3e956]/10 to-[#3ab666]/10 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div>
            <h1 className="text-4xl font-bold text-[#01645e]">إدارة الخبراء</h1>
            <p className="text-[#8b7632] text-lg">عرض وإدارة الخبراء في النظام</p>
          </div>
          <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { title: 'إجمالي الخبراء', value: stats.total, icon: Users, color: 'from-[#01645e] to-[#3ab666]' },
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              عرض جميع الخبراء المسجلين في النظام
            </CardDescription>
          </CardHeader>
          <CardContent>
            {filteredExperts.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-[#8b7632]">
                  {searchQuery ? 'لا توجد نتائج بحث' : 'لم يتم إضافة أي خبراء بعد'}
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <Badge className={expert.isActive ? 'bg-green-500' : 'bg-red-500'}>
                            {expert.isActive ? 'نشط' : 'غير نشط'}
                          </Badge>
                        </div>
        ...
        <Dialog open={inviteDialogOpen} onOpenChange={setInviteDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>دعوة خبير جديد</DialogTitle>
              <DialogDescription>
                إرسال دعوة لخبير جديد للانضمام إلى المنصة
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
