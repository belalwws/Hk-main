...
async function getDataStats() {
  try {
    ...
    // Try to count each table safely
    try {
      stats.users = await prisma.user.count();
    } catch (e) {
      log("⚠️ Users table not accessible, assuming 0", "yellow");
    }

    try {
      stats.hackathons = await prisma.hackathon.count();
    } catch (e) {
      log("⚠️ Hackathons table not accessible, assuming 0", "yellow");
    }

    try {
      stats.registrations = await prisma.hackathonRegistration.count();
    } catch (e) {
      log("⚠️ Registrations table not accessible, assuming 0", "yellow");
    }

    try {
      stats.landingPages = await prisma.hackathonLandingPage.count();
    } catch (e) {
      log("⚠️ Landing pages table not accessible, assuming 0", "yellow");
    }

    try {
      stats.formDesigns = await prisma.hackathonFormDesign.count();
    } catch (e) {
      log("⚠️ Form designs table not accessible, assuming 0", "yellow");
    }

    return stats;
  } catch (error) {
    log("⚠️ Error getting data stats, using defaults:", "yellow");
...
async function safeDatabaseUpdate() {
  try {
    ...
    execSync(
      "npx prisma db push --accept-data-loss=false --force-reset=false",
      {
        stdio: "inherit",
      }
    );
    ...
    try {
      ...
      execSync("npx prisma migrate dev --name auto_deploy", {
        stdio: "inherit",
      });
...
async function verifyDataIntegrity(beforeStats) {
  try {
    ...
    const checks = [
      ...
      {
        name: "Hackathons",
        before: beforeStats.hackathons,
        after: afterStats.hackathons,
      },
      {
        name: "Registrations",
        before: beforeStats.registrations,
        after: afterStats.registrations,
      },
      {
        name: "Landing Pages",
        before: beforeStats.landingPages,
        after: afterStats.landingPages,
      },
      {
        name: "Form Designs",
        before: beforeStats.formDesigns,
        after: afterStats.formDesigns,
      },
    ];
    ...
    for (const check of checks) {
      ...
      log(
        `${status} ${check.name}: ${check.before} → ${check.after} ${changeText}`,
...
async function ensureAdminExists() {
  try {
    ...
    try {
      ...
      log("⚠️ User table not ready yet, will create admin later", "yellow");
      return;
    }
    ...
    if (adminCount === 0) {
      ...
      try {
        ...
        log("⚠️ Could not create admin user (will try later):", "yellow");
...
async function main() {
  try {
    ...
    let beforeStats = await getDataStats();
    if (!beforeStats) {
      log("⚠️ Could not get initial stats, proceeding with caution", "yellow");
      // Don't fail, just proceed with empty stats
      beforeStats = {
        users: 0,
        hackathons: 0,
        registrations: 0,
    ...
    if (!integrityOk) {
      log("⚠️ Data integrity check had issues, but continuing...", "yellow");
      // Don't fail deployment for integrity issues in initial setup
    }
